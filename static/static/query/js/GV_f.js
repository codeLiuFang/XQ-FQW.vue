...
GV.paintPoints = function(data) {
	require([ "esri/layers/GraphicsLayer", "esri/geometry/Polyline",
			...
			"dojo/domReady!GV" ], function(GraphicsLayer, Polyline, lang, parser,
                                           ...
                                           array, ItemFileReadStore) {
		//var map = GV.getMap();
		var layer = GV.getLayer("pointLayer");
...
//			map.graphics.clear();
		...
		function paintsPoints(points, data, paths) {
			...
			for (var i = 0; i < points.length; i++) {
				...
				var graphic = paintPoint(points[i], data[i], symbol);
			...
			function(GraphicsLayer, Polyline, lang, parser, on, Map, Extent,
				...
				function init() {
					...
					var extent = {
					 ...
					 //var pointLayer = new GraphicsLayer({id :"pointLayer"});
					...
					addPointFeatureLyrs(FeatureLayer);
					GV.setMap(map);
				}
				...
				function addPointFeatureLyrs(FeatureLayer) {
...
GV.setMapExtent = function(){
	if (GE.getExtent()) {
		if(GE.getExtent().xmax < GE.getExtent().xmin){
			GE.getExtent().xmax = 180;
			GV.getMap().setExtent(GE.getExtent().expand(1));
		}
        GV.getMap().setExtent(GE.getExtent().expand(1));
	}
